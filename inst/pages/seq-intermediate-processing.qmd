...
Here, we load 10x Genomics Visium data from one postmortem human brain tissue section from the dorsolateral prefrontal cortex (DLPFC) brain region [@Maynard2021-DLPFC], which has undergone quality control and filtering in the preceeding @sec-seq-quality-control.
...
`r BiocStyle::Biocpkg("SpaNorm")` [@Salim2025-SpaNorm] was developed specifically for ST data, using a gene-wise model (e.g. negative binomial) in which variation is decomposed into a library size-dependent (technical) and -independent (biological) component.
