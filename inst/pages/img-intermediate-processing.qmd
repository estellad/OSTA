...
In this demo, we will continue processing the Xenium dataset from a human breast cancer biopsy section collected by @Janesick2023-high-res, which has already undergone quality control and minimal filtering; see @img-quality-control.
...
Here, we will run standard non-spatial and spatially-aware approaches; for a more comprehensive overview of methodology and tools, see @ind-dimensionality-reduction on dimension reduction and @ind-clustering on clustering. 
...
Library size-based normalization, as typical used for scRNA-seq data, has been shown to be problematic for ST data, especially so for targeted panels underlying current commercial imaging-based ST platforms [@Atta2024-gene-count; @Bhuva2024-library-size].
...
At this stage, we would typically perform selection of (e.g., highly variable) features; see @seq-intermediate-processing.
...
For comparison, we will also perform spatially-aware dimension reduction with `r BiocStyle::Biocpkg("BANKSY")` [@Singhal2024-BANKSY]; see @ind-dimensionality-reduction.
