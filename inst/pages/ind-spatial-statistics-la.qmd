...
Again, we will use the human breast cancer Xenium data set [@Janesick2023-high-res] in a `SpatialFeatureExperiment` object available through `r BiocStyle::Biocpkg("SFEData")`. 

We will use the `r BiocStyle::Biocpkg("Voyager")` package [@Moses2023-Voyager] which provides convenient wrappers around classic geospatial R packages such as `r BiocStyle::CRANpkg("spdep")` [@Pebesma2023-book-spatial-data-science]. For more details, please consult the authors' comprehensive [vignettes](https://pachterlab.github.io/voyager/index.html). 

This part is based on the following vignette of the [*pasta* resource](https://robinsonlabuzh.github.io/pasta/00-overview-latSOD.html) [@Emons2025-pasta].
...
```{r weight_matrix, warning = FALSE, fig.width=25, fig.height=10}
colGraph(sfe, "knn6") <-
...
```{r plot_moran_genes, fig.width=25, fig.height=10}
...
```{r local_moran, fig.width=25, fig.height=10}
sfe <- runUnivariate(sfe,
  type = "localmoran",
  features = topGenes,
  colGraphName = "knn6",
...
```{r local_moran_pvals, fig.width=25, fig.height=10}
plotLocalResult(sfe,
                name = "localmoran",
                features = topGenes,
...
```{r local_lees_l, fig.width=20, fig.height=10}
sfe <- runBivariate(
  sfe,
  "locallee",
  colGraphName =  "knn6",
