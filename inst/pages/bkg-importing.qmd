# Data importing {#sec-bkg-importing}
...
In addition, `r BiocStyle::Biocpkg("STexampleData")` provides access to several pre-formatted datasets from several platforms in `SpatialExperiment` and `SingleCellExperiment` formats, which are stored on Bioconductor's `r BiocStyle::Biocpkg("ExperimentHub")` resource, and are loaded within several examples in this book.
