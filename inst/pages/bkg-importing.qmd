...
<!-- To do: maybe move all loading examples here and save objects for re-use in later chapters, so only need to load each dataset once - may speed up build time of book -->
...
- Visium datasets using `TENxVisium()` function from `r BiocStyle::Biocpkg("VisiumIO")` package [here](@sec-seq-deconvolution-load-data), [here](@sec-seq-workflow-visium-crc-load-data), and [here](@sec-crs-workflow-xenvis-load-data)

- Visium HD dataset using `TENxVisiumHD()` function from `r BiocStyle::Biocpkg("VisiumIO")` package [here](@sec-seq-workflow-visium-hd-load-data)

- Xenium datasets using `readXeniumSXE()` function from `r BiocStyle::Biocpkg("SpatialExperimentIO")` package [here](@sec-img-workflow-xenium-load-data) and [here](@sec-crs-workflow-xenvis-load-data)
