...
Reader functions from several Bioconductor packages can be used to import data from raw files into a `SpatialExperiment` object (or derivatives thereof; see @sec-bkg-data-infrastructure) in R, including:
...
Examples showing how to read in raw data into R using functions from the Bioconductor packages listed above are included in the workflow chapters in this book (e.g. @sec-seq-workflow-visium-crc, @sec-seq-workflow-visium-hd, @sec-img-workflow-xenium, and @sec-crs-workflow-xenvis). The datasets are described in more detail in @sec-bkg-example-datasets.
