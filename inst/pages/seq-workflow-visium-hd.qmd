...
pal <- hcl.colors(length(unique(unlist(cd[lab]))), "Spectral")
lapply(lab, \(.) {
        plotSpatial(vhd8, annotate=., point_size=0.1, shape = 15) + ggtitle(.)
    }) |>
    wrap_plots(nrow=1, guides="collect") &
    scale_color_manual(NULL, values=pal) &
    theme(legend.key.size=unit(0, "lines"))
...
p1 <- plotVisium(vhd16, annotate="libsize", shape = 22,
           zoom = TRUE, show_axes = TRUE, point_size = 1.3) + 
...
p2 <- plotVisium(vhd16, annotate="libsize", shape = 22,
           zoom = TRUE, show_axes = TRUE, point_size = 1.3) + 
...
(plotSpatial(vhd8, annotate="cluster", point_size=0.05, shape = 15,
             pal=unname(pals::kelly())) + ggtitle("8 µm")) |
(plotSpatial(vhd16[, !is.na(vhd16$cluster)], annotate="cluster", point_size=0.1, 
             shape = 15, pal=unname(pals::kelly())) + ggtitle("16 µm")) 
```
...
plotVisium(vhd8, spots=FALSE, shape=22) + cov + box +
    ggtitle("grey: data coverage\n black: zoomed region") + 
plotVisium(vhd8, point_size=0.8, zoom=TRUE, shape=22) + .lim(vhd8r) +
    ggtitle("8 µm bins in black box") + 
plotVisium(vhd16, point_size=1.6, zoom=TRUE, shape=22) + .lim(vhd8r) +
    ggtitle("16 µm bins in black box") + 
plot_layout(nrow=1, widths=c(1.5, 1, 1)) & facet_null() &
    theme(plot.title=element_text(hjust=0.5, vjust=0.5))
```
...
plotSpatial(.vhd16, annotate="sum_log", point_size=0.5, shape=15) + 
  ggtitle("log-library size") +
plotSpatial(.vhd16, annotate="subsets_mt_percent", point_size=0.5, shape=15) + 
...
plotSpatial(.vhd16, annotate="sum_outliers", point_size=0.8, shape=15) + 
  ggtitle("low_lib_size") +
plotSpatial(.vhd16, annotate="detected_outliers", point_size=0.8, shape=15) + 
  ggtitle("low_n_features") +
plotSpatial(.vhd16, annotate="discard", point_size=0.8, shape=15) + 
  ggtitle("discard") +
plot_layout(nrow=1, guides="collect") & theme(
    plot.title=element_text(hjust=0.5),
    legend.key.size=unit(0, "lines")) &
...
ps <- lapply(gs, \(.) plotSpatial(.vhd16, annotate=., shape=15, point_size=0.8,
...
#     scale_color_gradientn(colors=rev(hcl.colors(9, "Rocket")))
lapply(names(ws), \(.) 
    plotSpatial(.vhd16, annotate=., point_size=0.3, shape=15)) |>
    wrap_plots(nrow=2, guides="collect") & theme(
    legend.key.width=unit(0.5, "lines"),
    legend.key.height=unit(1, "lines")) &
    scale_color_gradientn(colors=rev(hcl.colors(9, "Rocket")))
...
#| code-fold: true
plotVisium(.vhd8, annotate=".DeconLabel1", zoom =TRUE, point_size=0.8, shape=22) + 
  ggtitle("8 µm") + plot_spacer() +
plotVisium(.vhd16, annotate=".DeconLabel1", zoom=TRUE, point_size=1.6, shape=22) + 
  ggtitle("16 µm") +
...
#| code-fold: true
plotVisium(.vhd16, annotate="cluster", zoom=TRUE, shape=21,
    point_size=1.6, pal=unname(pals::kelly())) + 
    plot_spacer() +
plotVisium(.vhd16, annotate="Banksy", zoom=TRUE, shape=21,
    point_size=1.6, pal=unname(pals::kelly())) + 
    plot_spacer() +
plotVisium(.vhd16, annotate=".DeconLabel1", zoom=TRUE, shape=21,
...
Here, we aim to determin whether the expression level of the tumor marker changes in proximity to fibroblast. Using FTH1 as an example, we observe a decrease in FTH1 expression in tumor bins as the distance to fibroblast increase. This trend could be visualized using a scatter plot. 
