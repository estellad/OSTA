...
Next, we can perform differential gene expresison (DGE) analysis to identify markers for each cluster; c.f. @sec-ind-feature-set-signatures. We also compute the cluster-wise mean expression of selected markers:
...
In this workflow, we demonstrated how to perform a standard analysis pipeline on a subset of Visium HD data binned at 16 µm. Most methods and visualization techniques developed for Visium can be applied to Visium HD. However, more sparsity is expected, as each data point (e.g. 2, 8, or 16 µm) has smaller area compared to Visium (55 µm). Each bin cannot be interpreted as a cell, but rather, a segmentation-free approach is applied here. Below are some resources and news for Visium HD to consider and incorporate into your pipeline:

- 10x Genomics [news](https://www.prnewswire.com/news-releases/10x-genomics-unveils-innovation-roadmap-at-agbt-general-meeting-advancing-leadership-in-large-scale-single-cell-and-spatial-analysis-302382827.html) on morphology-driven segmentation from H&E enabled in CytAssist for Visium HD, by assigning transcripts to individual cells. Notably, Space Ranger v4 provides a segmented, cell-level count matrix under `segmented_outputs`, in addition to the previous `binned_outputs` at 2, 8, and 16 µm, simplifying additional cell segmentation using `bin2cell` or `StarDist`. Please see the following section for some cell-level Visium HD analysis illustration. However, a comparative study of segmentation methods on Visium HD data has yet to be conducted. 
...
For a more detailed cell-level analysis on Visium HD, please refer to the EuroBioC2025 OSTA workshop [tutorial page](https://estellad.github.io/EuroBioC2025OSTAWorkshop/index.html).
