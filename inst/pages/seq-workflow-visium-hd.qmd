...
(res <- runRctd(.vhd16, .sce, cell_type_col="Level1", 
                max_cores=5, rctd_mode="doublet"))
```
...
From the above heatmap, cluster 7 is mostly immune cells with endothelial; cluster 9 matches B cell bins; cluster 3 and 6 are likely intestinal epithelial and smooth muscle, respectively; fibroblast is common in cluster 5; tumor spans cluster 2 and 8. 

## Neighborhood analysis

Here, we want to quantify the abundance between *T cells*, *B cells*, or 
*Myeloid* bins to *Fibroblast* or *Tumor* bins in 8 µm zoomed region.
...
In this region, we observe more fibroblast than tumor bins around each of the immune bins.
However, note that in this zoomed region, we have nearly 1.4 times more fibroblast bins compared to tumor bins. 
...
After correction, we see amplified differences in the relative abundance of 
fibroblast bins compared to tumor bins around each immune cell type in the 
zoomed region. 

The same analysis steps could be applied to the (unfiltered) 16 μm resolution 
