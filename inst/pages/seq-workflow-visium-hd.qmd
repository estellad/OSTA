...
[Please see the following section for some cell-level Visium HD analysis guide.]{.aside}
...
We can read in the segmented output:
...
dir.create(td <- tempfile())
unzip(pa, exdir=td)
vhdcell <- TENxVisiumHD(
    format = "h5",
    segmented_outputs = file.path(td,"segmented_outputs"),
    images = "hires")|>
    import()
rownames(vhdcell) <- make.unique(rowData(vhdcell)$Symbol)
vhdcell
```

To orient the polygons in the same direction as the previous plots, we will need to manually flip the y-axis of cell segmentation masks.   
```{r vhdcellflip}
polygons <- VisiumIO::flip_sf_Y(sf = metadata(vhdcellsub)$cellseg, 
                                type = "POLYGON",
                                img_height = nrow(imgRaster(vhdcell)),
                                scalef = imgData(vhdcellsub)$scaleFactor)

```

Subset the Visium HD data to region of interest as previously in the bin-level:
```{r}
bbox <- st_bbox(c(xmin = 4214.937, xmax = 4465.761, 
                    ymin = 2979.286, ymax = 3211.817))
bbox <- st_as_sfc(bbox)
vhdpo <- metadata(vhdcell)$cellseg
vhdposub <- vhdpo[st_intersects(vhdpo, bbox, sparse = FALSE), ]

vhdcellsub <- 
```

Visualization gene expression in the polygons
```{r vhdcellplot}
ggplot() + 
  geom_sf() + 
  theme_void()
...
Compared to binned analysis, Visium HD with cell-level segmentation is more similar to Xenium data, but with full-transcriptome coverage. Most analytical methods are transferable; however, at single-cell resolution, deconvolving each data point into multiple cell types should be adapted using label transfer or unsupervised clustering to assign single-cell type labels. For example, after clustering, we identified three differentially expressed genes:

```{r fig-vhdcell-ge, echo = FALSE, out.width = "50%", fig.cap= "Differential marker gene expressions spatially visualized as polygons"}
knitr::include_graphics("../images/VisiumHD_gene_expression_polygon.png")
```


For detailed cell-level Visium HD workflow, please refer to the EuroBioC2025 OSTA workshop [tutorial page](https://estellad.github.io/EuroBioC2025OSTAWorkshop/index.html).
