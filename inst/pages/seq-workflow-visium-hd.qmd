...
These data come with bin-level annotations from deconvolution estimates by `RCTD`, implemented in `r BiocStyle::Githubpkg("ggrajeda/spacexr")`. Specifically, two sets of labels are available that correspond to the most and second most frequent of 38 cell types per bin, respectively:
...
Some global filtering based on library size is required to remove bins overlaying empty tissue. 

```{r vhd16-libsize}
vhd16$libsize <- colSums(counts(vhd16))
```

We can visualize such bins by zooming into an empty tissue region. On the left hand side, one region has an obvious gap that shows low counts. Informed by H&E, bins located above tissue gaps should not have a biological interpretation, and the low counts in those bins are likely due to ambient RNA or transcript spillover, and thus should be removed from downstream analysis. 

Based on a [discussion](https://github.com/10XGenomics/HumanColonCancer_VisiumHD/issues/28) with the original author at 10x Genomics, in this dataset, 8 µm bins with library size lower than 100 are removed. Since we expect a 4 fold increase in library size across all bins in 16 µm, we can set the filtering threshold to 400. 

```{r vhd16-empvis, fig.width=8, fig.height=2.5, message=FALSE, warning=FALSE}
#| code-fold: true
p1 <- plotVisium(vhd16, annotate="libsize", 
...
p2 <- plotVisium(vhd16, annotate="libsize", 
           zoom = TRUE, show_axes = TRUE, point_size = 1.3) + 
    xlim(c(422, 442)) + ylim(c(318, 330)) + 
...
Clustering results generated by Space Ranger are provided for these two resolutions.
...
We can visualize the given clustering results at 8 and 16 µm. Later, we will compare the given clustering result at 16 µm with Banksy.

```{r clusvis, fig.width=12, fig.height=6, message=FALSE}
#| code-fold: true
(plotSpots(vhd8, annotate="cluster", point_size=0.05, 
           pal=unname(pals::kelly())) + ggtitle("8 µm")) |
(plotSpots(vhd16[, !is.na(vhd16$cluster)], annotate="cluster", point_size=0.1, 
...
plotVisium(vhd16, point_size=1.6, zoom=TRUE) + .lim(vhd8r) +
...
Next, we can perform differential gene expresison (DGE) analysis to identify markers for each cluster; c.f. @sec-crs-spatially-variable. We also compute the cluster-wise mean expression of selected markers:
...
```{r plt-mgs-hm, message=FALSE, fig.width=11, fig.height=3}
#| code-fold: true
...
We observe a clear agreement in deconvolution at 8 and 16 µm resolution, yet 8 µm better captures fine structure, such as the string shape of endothelial and fibroblast bins extended into the upper left tumor region. 
...
We see that Banksy delineates very well the tissue borders, while deconvolution can directly inform the biology meaning behind clusters. Their concordance can be visualized with a heatmap:
...
We can also investigate the "cell-cell-marker" relationship based on the minimal distance between two cell types within a radius. The minimal distance matrix between all pairwise cell types is stored in the reduced dimension as previously the abundance matrix. 
```{r vhd8-getdis}
.vhd8 <- getDistances(.vhd8, cellType=".DeconLabel1", imageID="sample_id",
                      spatialCoords=names(xy), maxDist = 200, nCores=4)
```

Here, we want to see if Tumor marker changes its expression level in the proximity of Fibroblast. Using FTH1 as an example, we observe a decrease of FTH1 expression in Tumor bins as distance to Fibroblast increase. Such trend could be plotted as a scatter plot. 
