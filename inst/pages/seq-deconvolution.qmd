...
[Note that RCTD can also be adapted to VisiumHD data with `doublet_mode="doublet"`, as demonstrated by [@Oliveira2024] and @sec-seq-workflow-visiumhd.]{.aside}
By default, `run.RCTD()`'s `doublet_mode="doublet"`, specifies at most two subpopulations coexist in a data unit, such as a "spot"; here, we set `doublet_mode="full"` in order to allow for an arbitrary number of subpopulations to be fit instead.

```{r decvis, message=FALSE, warning=FALSE, results='hide'}
# prep spatial data (Visium)
xy <- data.frame(spatialCoords(vis))
.vis <- SpatialRNA(xy, counts(vis))
# prep reference data (Chromium)
ids <- factor(sce$Annogrp)
names(ids) <- colnames(sce)
ref <- Reference(counts(sce), ids)
# run 'RCDT' deconvolution
res <- create.RCTD(.vis, ref, max_cores=10)
...
ws_rctd <- normalize_weights(res@results$weights)
# add proportion estimates as spot metadata
ws_rctd <- data.frame(as.matrix(ws_rctd))
...
Another method could be used is CARD. First, we realized delayed count matrices and rename spatial coordinate columns to make the input data compatible with CARD [Note: RCTD handles the conversion internally with spacexr::check_counts()]{.aside}
...
Next, we perform the `CARD` deconvoluition. Here we demonstrate `CARD`'s interoperability with `SingleCellExperiment` and `SpatialExperiment`. [Note: `CARD` can also take a reference matrix, a reference cell type annotation column, a spatial count matrix, and a spatial coordinates data frame as separate items in `sc_count`, `sc_meta`, `spatial_count`, and `spatial_location`, respectively. Yet, we encourage simplify such process with using existing Bioconductor classes.]{.aside} The deconvolution result matrix is already normalized such that the sum of cell type proportions for each spot is equal to 1. 
...
# Order cell type names alphabetically as RCTD
ws_card <- ws_card[, colnames(ws_rctd)]
...
#| code-fold: true
plotSpatial <- function(plt_df, annotate="CT"){
  stopifnot(c(annotate, "x", "y") %in% names(plt_df))
  
  ggplot(plt_df, aes(x=x, y=y, color=get(annotate))) + 
...
plot_per_ct_ws <- function(ws){
  ws
  p <- lapply(names(ws), \(.) 
    plotSpatial(, annotate=., point_size = 0.05)) |>
