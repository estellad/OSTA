...
::: {.callout-note title="Visium subset" collapse="true"}
```{r subse, message=FALSE}
# subset Visium spots for faster runtime in this demo
# selecting region containing tumor and other tissue of interest
keep_r <- vis$array_row > 10 & vis$array_row < 70
keep_c <- vis$array_col > 10 & vis$array_col < 70
vis <- vis[, keep_r & keep_c]
```

```{r visanno_subse, message = FALSE, fig.width = 4.5, fig.height = 3.5}
#| code-fold: true
plotCoords(vis, 
           annotate = "anno", point_size = 1, 
           pal = unname(pals::trubetskoy())) + 
...
```{r plt-pcs-xy, message = FALSE, fig.width = 7, fig.height = 5}
#| code-fold: true
pcs <- reducedDim(vis, "PCA")
