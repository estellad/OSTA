...
Moran's $I$ can be interpreted as the Pearson correlation between the value at a certain location and the average values of its neigbors. The global value is a weighted average of the respective local values [@Moran1950].
...
Here, we will use the this to quantify the tendency of clusters (non-spatial) to co-localize in their relative neigbhorhood. Note that the output contains z-scores and no p-values such that a user-defined threshold can be applied.
