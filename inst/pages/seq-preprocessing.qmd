...
![FASTQ file format details from the BGI STOmics Stereo-seq user manual. Here both Read 1 and Read 2 are shown while for real data these would be in two separate files as described above.](seq-preprocessing-Stereo-seq_Analysis_Workflow_Manual_fastq_File_Format.png){#fig-fastq_format}
...
Although reads contain most of the information we need to construct a count matrix, the data structure we need for downstream analysis, often we also need a 'mapping' file which contains the information needed to interpret the Coordinate IDs (CIDs) in the barcode sequences contained in Read 1 as spatial locations. For some sST platforms the mapping from barcodes to locations is fixed and no additional files are needed, for others the mapping is random and the vendor will have provided a file for this mapping.

The process of mapping reads based on their CIDs to spatial locations is known as barcode deconvolution.

::: {.callout-tip collapse="false"}
## Example Barcode Deconvolution Methodology

For the sake of this example we will assume that CIDs are distributed randomly, as is the case with Stereo-seq, Curio Seeker, and Visium HD (!!as of April 2025 details not confirmed by 10X!!). As such we can consider the barcode deconvolution step as having the following inputs and outputs:
...
Alignment is performed by a software aligner such as `STAR`[^3], `minimap2`[^4], or `Rsubread`[^5]. SpaceRanger and SAW are vendor pipelines for Visium and Stereo-seq respectively and both make use of `STAR`[^3] for alignment.
...
![Read Mapping Plot](seq-preprocessing-stpipe_read_mapping_plot.png){#fig-read_mapping}

![UMI Duplication Distribution](seq-preprocessing-stpipe_umi_duplication_plot.png){#fig-umi_duplication}
...
* Curio Seeker Bioinformatic Pipeline[^8]
...
StPipe[^9] is a sST data preprocessing tool designed to be platform agnostic, and to unify the preprocessing of sST data from a wide variety of spatial technology solutions such as 10x Visium, BGI Stereo-seq, and Curio Slide-seq.
...
![Workflow diagram for stPipe including function calls and data objects with downstream analysis options highlighted.](seq-preprocessing-stpipe_workflow_diagram_preprint.jpg){#fig-stpipe_workflow}
...
[^4]: minimap2 aligner <https://github.com/lh3/minimap2> Heng Li, Minimap2: pairwise alignment for nucleotide sequences, Bioinformatics, Volume 34, Issue 18, September 2018, Pages 3094–3100, <https://doi.org/10.1093/bioinformatics/bty191>
[^5]: Rsubread <https://bioconductor.org/packages/release/bioc/html/Rsubread.html>
[^6]: Space Ranger <https://www.10xgenomics.com/support/software/space-ranger/latest>
[^7]: SAW <https://en.stomics.tech/products/stomics-software/stomics-offline-software/list.html>
[^8]: Curio Seeker <https://curiobioscience.com/seeker/>
[^9]: stPipe <https://github.com/mritchielab/stPipe/>