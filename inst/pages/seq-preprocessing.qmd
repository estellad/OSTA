# Preprocessing {#sec-seq-preprocessing}
...
![FASTQ file format details from the BGI STOmics Stereo-seq user manuals. Here both Read 1 and Read 2 are shown while for real data these would be in two separate files as described above.](../images/seq-preprocessing-Stereo-seq_Analysis_Workflow_Manual_fastq_File_Format.png){#fig-fastq_format}
...
Alignment is performed by a software aligner such as `STAR` [@Dobin2013-STAR], or `r BiocStyle::Biocpkg("Rsubread")` [@Liao2019-Rsubread]. SpaceRanger and SAW are vendor pipelines for Visium and Stereo-seq respectively and both make use of `STAR` for alignment.
...
`r BiocStyle::Githubpkg("mritchielab/stPipe")` [@Xu2025-stPipe] is a sST data preprocessing tool designed to be platform agnostic, and to unify the preprocessing of sST data from a wide variety of spatial technology solutions such as 10x Visium, BGI Stereo-seq, and Curio Slide-seq.
