...
Different data structures, although standardized within a given framework, make switching between languages and tools somewhat cumbersome. In the realm of single-cell and spatial omics, all Bioconductor tools are built around `r BiocStyle::Biocpkg("SummarizedExperiment")`-derived classes, while [Seurat](https://satijalab.org/seurat/) [@Hao2023-Seurat], [Giotto](https://giottosuite.com/) [@Chen2023-Giotto-Suite], and `r BiocStyle::Githubpkg("BIMSBbioinfo/VoltRon")` each rely on their own object definitions. In Python, [Scanpy](https://scanpy.readthedocs.io/) [@Wolf2018-Scanpy] and [Squidpy](http://squidpy.readthedocs.io/) [@Palla2022-Squidpy] use [AnnData](https://anndata.readthedocs.io/). Attempts to alleviate the problem are being made -- e.g. `r BiocStyle::Biocpkg("zellkonverter")`, [anndataR](https://github.com/scverse/anndataR) [@Deconinck2025-anndataR], and functions from `Seurat` allow for conversion between Python's `AnnData` and R/Bioconductor's `r BiocStyle::Biocpkg("SingleCellExperiment")` or `r BiocStyle::Biocpkg("SpatialExperiment")`.
