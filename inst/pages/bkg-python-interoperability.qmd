...
We here set up a virtual environment using [reticulate](https://rstudio.github.io/reticulate/) to interact with Python `anndata` objects in R. 
...
```{r conda-settings, eval=FALSE, warning=FALSE, message=FALSE, results="hide"}
...
```{r install-miniconda, eval=FALSE, warning=FALSE, message=FALSE, results="hide"}
...
txt <- file.path(dirname(getwd()), "extdata", "requirements.txt")
virtualenv_create(env <- "OSTA", python="3.8.18") 
virtualenv_install(env, requirements=txt)
use_virtualenv(env, required=TRUE)
#virtualenv_install(envname=env, requirements=txt) 
```
...
```{r conda-create, eval=FALSE, warning=FALSE, message=FALSE, results="hide"}
...
```{r ld-lib-path, eval=FALSE}
lib <- file.path(miniconda_path(), "lib")
system(paste0("export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:", lib))
```
...
```{r deps-conda-forge, eval=FALSE, warning=FALSE, message=FALSE, results="hide"}
...
```{r deps-pypi, eval=FALSE, warning=FALSE, message=FALSE, results="hide"}
...
```{r python-reticulate, eval=FALSE}
