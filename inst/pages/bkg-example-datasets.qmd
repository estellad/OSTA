...
In can happen that we change, add, or remove data from the OSF repositorying underlying `OSTA.data`. Should you ever run into any issues that might be related to this, we suggest removing affected cache resource(s) as follows, and retrieving these data anew.

```{r bfc, eval=FALSE}
library(BiocFileCache)
bfc <- BiocFileCache()
# dataset identifier pattern
pat <- "Xen.*Oliveira"    
# query cached files by 'pat'
que <- bfcquery(bfc, pat) 
# clear matching resource(s)
