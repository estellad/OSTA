...
These datasets have been deposited in an Open Storage Framework (OSF) repository [here](https://osf.io/5n4q3), and can be easily queried and downloaded using functions from the `r BiocStyle::CRANpkg("osfr")` package. For convenience, we have implemented the `r BiocStyle::Biocpkg("OSTA.data")` package to:
...
In addition, several datasets are available from the `r BiocStyle::Biocpkg("STexampleData")` package as pre-formatted `SpatialExperiment` and `SingleCellExperiment` formats. These data objects are stored on Bioconductor's `r BiocStyle::Biocpkg("ExperimentHub")` resource, and can be loaded within R code examples by querying `ExperimentHub` or using loader functions provided in the `STexampleData` package.

```{r list-stexampledata, message=FALSE, warning=FALSE}
library(STexampleData)

# help file listing datasets
# ?STexampleData
```
