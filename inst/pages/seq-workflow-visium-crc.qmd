...
plotSpatial(spe, annotate="log_sum") + ggtitle("log-library size") +
plotSpatial(spe, annotate="subsets_mt_percent") + ggtitle("% mitochondrial") +
...
#| code-fold: true
lapply(names(ol), \(.) 
    plotSpatial(spe, annotate=.) + ggtitle(.)) |>
...
(res <- runRctd(rctd_data, max_cores=6, rctd_mode="full"))
```
...
lapply(top, \(.) {
    ...
    plotSpatial(spe, annotate=.) # plotting
}) |> 
    # arrange & prettify
