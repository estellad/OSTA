...
In Bioconductor, the primary class for data from single-cell experiments is `r BiocStyle::Biocpkg("SingleCellExperiment")` [@Amezquita2020-OSCA] (SCE).
...
`r BiocStyle::Biocpkg("SpatialExperiment")` (SPE) [@Righelli2022-SpatialExperiment] is the core data class used in this book. 
...
A schematic of the `SpatialExperiment` object structure is shown in @fig-spatialexperiment. 
...
```{r fig-spatialdata, echo = FALSE, out.width = "80%", fig.cap = "Overview of the scverse/Python `SpatialData` framework."}
