...
Ok, let's get started! First we need to re-create the `spe` object from the @sec-seq-workflow-dlpfc in the previous chapter. That chapter included code for visualizing results along the way, which we'll skip here. Thus, we'll use the following packages in addition to `spatialLIBD`:
...
We can now run the following R code to re-make the `spe` object from the @sec-seq-workflow-dlpfc. This will take a bit of time.
...
* we re-used the `spe` object from the @sec-seq-workflow-dlpfc chapter,
