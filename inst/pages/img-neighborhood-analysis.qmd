...
```{r imcRtools-calc, message=FALSE, eval=FALSE}
library(imcRtools)
...
```{r imcRtools-plot, fig.width=9, fig.height=3, fig.cap="Tissue plot with cells colored by cluster (left) and spatial context (right) based on $k$-means clustering of cluster frequencies among each cell's (Euclidean) $k$NNs.", eval=FALSE}
#| code-fold: true
...
```{r hoodscanR-plot-mets-k, fig.width=7, fig.height=2.5, fig.cap="Boxplot of entropy and perplexity, stratified by cluster assignment and z-scaled across cells (capped at 2 SDs).", eval=FALSE}
#| code-fold: true
