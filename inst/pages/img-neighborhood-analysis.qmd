...
`r BiocStyle::Biocpkg("scider")` [CITATION] defines spatial domains as regions of interest (ROIs) while preserving the tissue structure though spatial density analysis. 
...
```{r scider-load-package-and-data}
#| message: false
...
Firstly, we need to add cell type annotations to `colData`, where cell type annotations were obtained from the original publication [CITATION-Janesick2023]. 
...
plotDensity(spe, coi = coi, probs = 0.5) +
  ggtitle("Spatial density of tumour cells")
```

ROI detection is performed using a graph-based approach as described in Li et al (2025) [CITATION]. 
...
plotContour(spe, coi = coi, line.width = 0.5)
```
...
plotCellCompo(spe, contour = coi, self.included = FALSE)
```
...
plotCellCompo(spe, contour = coi, self.included = FALSE, roi = coi)
```
...
We recommend to perform such DE analyses using the `voomLmFit()` pipeline [CITATION BELOW] or the quasi-likelihood pipeline in `r BiocStyle::Biocpkg("edgeR")` [CITE EDGER4.0].
Examples of DE and pathway analyses can be found in Li et al (2025) [CITATION]. 
More case studies will also be available at https://github.com/ChenLaboratory/scider. 

### REFERENCES (TO DELETE)

scider - 
Li, M., Liu, N., Nguyen, Q.H. and Chen, Y., 2025. Preserving tissue structure through density-based spatial analysis with scider. bioRxiv, pp.2025-09.

voomLmFit - 
Baldoni, P.L., Chen, L., Li, M., Chen, Y. and Smyth, G.K., 2025. Dividing out quantification uncertainty enables assessment of differential transcript usage with diffSplice. bioRxiv, pp.2025-04.

edgeR - 
Chen, Y., Chen, L., Lun, A.T., Baldoni, P.L. and Smyth, G.K., 2025. edgeR v4: powerful differential analysis of sequencing data with expanded functionality and improved support for small counts and larger datasets. Nucleic Acids Research, 53(2), p.gkaf018.
