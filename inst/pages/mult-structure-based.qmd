...
First, we plot a few selected fields of view (FOV).
...
allIslets |> 
  ...
  ggplot(aes(x = patient_id, y = value^(1/4), fill = patient_stage)) + 
  geom_violin() + 
  geom_boxplot(aes(fill = NULL), width = 0.3) + 
  geom_jitter(size = .5) +
...
To account for this, we will use mixed linear models with random effects for the patient and the individual slides (image name). The `r BiocStyle::CRANpkg("lme4")` package will be use for fitting linear mixed effects models [@batesFittingLinearMixedEffects2015] and `r BiocStyle::CRANpkg("lmerTest")` for p-value calculations [@kuznetsovaLmerTestPackageTests2017].
