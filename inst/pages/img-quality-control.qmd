...
# cos1 <- SpaceTrooper::readCosmxSPE(td)
cos <- readCosmxSXE(td)

# TODO: needed for `plotCellsFovs()`
metadata(cos)$fov_dim <- c(xdim = 4256, ydim = 4256)

# add cell boundary polygons
# fnm <- file.path(td, "polygons.csv.gz")
# pol1 <- readPolygonsCosmx(fnm)

pol <- readPolygonsCosmx(metadata(cos)$polygons, type = "parquet")
...
```{r cos-plot-qc, warning=FALSE, fig.width=12, fig.height=9}
#| code-fold: true
# TODO: compute ctrl_total_ratio
# TODO: scuttle::addPerCellQC(, use_altexp = TRUE) -> when QC
df <- data.frame(colData(cos))
vs <- "Mean"
