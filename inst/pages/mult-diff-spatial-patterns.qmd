...
In this chapter, for computational reasons, we use 3 stages -- 2, 10, and 20 days post injury (DPI) -- and only two sections per stage. The analysis framework uses pre-computed spatial domains generated with `r BiocStyle::Biocpkg("Banksy")` [@Singhal2024-BANKSY]. For performing clustering with `Banksy`, refer to @sec-ind-clustering. For preprocessing details of this dataset, see the `r BiocStyle::Biocpkg("muSpaData")` Bioconductor package.
