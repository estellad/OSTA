...
The book covers both preprocessing and downstream analyses, so the starting point can be raw spatial omics datsets, or those that already spot/cell-level expression matrix and set of spatial coordinates as the main inputs. Preprocessing procedures can vary from platform to platform and we aim to cover here some of the main pipelines for multiple platforms.
